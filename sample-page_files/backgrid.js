/*
  ...
  08/21/2013 cpower - fixed bug on SelectCellEditor to correctly retrieve options
  08/30/2013 cpower - passed model to toRaw / fromRaw
  10/30/2013 cpower - modified sort to sort by optional field passed in column definition
  02/10/2015 gdfesta - Fix for moveToNextCell because of bug on backgrid (https://github.com/gbedardsice/backgrid/commit/391540bc9e5851901192da7e1552f6d41b0188a7#diff-f0c01727e5628483090afb5bc8c36be6R267)
*/
...
var InputCellEditor = Backgrid.InputCellEditor = CellEditor.extend({
  ...
  */
  render: function () {
    this.$el.val(this.formatter.fromRaw(this.model.get(this.column.get("name")), this.model));
    return this;
  },
...
var Cell = Backgrid.Cell = Backbone.View.extend({
  ...
  */
  enterEditMode: function () {
    var model = this.model;
    var column = this.column;
    var editable = _.isFunction(column.get("editable")) ? column.get("editable").apply(this) : column.get("editable");
  ...
  */
  saveOrCancel: function (e) {
    ...
    if (command.save() || command.moveLeft() || command.moveRight() || command.moveUp() ||
      ...
      e.preventDefault();
      e.stopPropagation();
      var val = formatter.toRaw($el.prop("checked"), this.model);
  ...
  */
  render: function () {
    ...
    this.$el.append($("<input>", {
      tabIndex: -1,
      type: "checkbox",
      checked: this.formatter.fromRaw(this.model.get(this.column.get("name")), this.model)
    }));
    this.delegateEvents();
    return this;
  }
...
var SelectCellEditor = Backgrid.SelectCellEditor = CellEditor.extend({
  ...
  */
  render: function () {
    ...
    var selectedValues = this.formatter.fromRaw(this.model.get(this.column.get("name")), this.model);
  ...
  */
  save: function (e) {
    var model = this.model;
    var column = this.column;
    model.set(column.get("name"), this.formatter.toRaw(this.$el.val(), this.model));
...
var SelectCell = Backgrid.SelectCell = Cell.extend({
  ...
  */
  initialize: function (options) {
    ...
    this.listenTo(this.model, "backgrid:edited", this.readFromCompany);
  },
  ...
  */
  render: function () {
    ...
    // cpower: fix for select options
    var optionValues = _.result(this, "optionValues");
    var rawData = this.formatter.fromRaw(this.model.get(this.column.get("name")), this.model);
  ...
  */
  ...
  _resetCellDirection: function (sortByColName, direction, comparator, collection) {
    if (collection == this.collection) {
      if (sortByColName !== (this.column.get("sortName") || this.column.get("name"))) this.direction(null);
  ...
  onClick: function (e) {
    ...
    var columnName = this.column.get("sortName") || this.column.get("name");
    ...
    if (command.moveUp() || command.moveDown() || command.moveLeft() ||
      ...
      if (command.moveUp() || command.moveDown()) {
        ...
        for (var offset = i * l + j + (right ? 1 : -1);
          ...
          var editable = _.isFunction(column.get("editable")) ? column.get("editable").apply(this) : column.get("editable");
          if (cell.column.get("renderable") && editable) {
            cell.enterEditMode();
            break;
          }
        }
